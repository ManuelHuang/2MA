...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self, period_short=3, period_long=15, buy_threshold=2, sell_threshold=-2, trail_percentage_buy=0.05,
                 trail_percentage_sell=0.05, back_stop_order_percentage_buy=1.05, back_stop_order_percentage_sell=1.05):
        ...
        self.period = period_long
        self.sma_short = bt.indicators.SMA(period=period_short)
        self.sma_long = bt.indicators.SMA(period=period_long)
        self.dif_sma = self.sma_short - self.sma_long
        self.dif_dif_sma = self.dif_sma(0) - self.dif_sma(-1)
        self.buy_threshold = buy_threshold
        self.sell_threshold = sell_threshold

        self.trail_percentage_sell = trail_percentage_sell  # 测试范围 0.015-0.03，步长0.001
        self.trail_percentage_buy = trail_percentage_buy
    ...
    def next(self):
        if self.position.size == 0 and len(self) > self.period and (len(self) - self.last_trade_length) > 60:

            self.cancel_all_orders()

            self.trade_amount = round(self.cerebro.broker.getcash() / self.dataopen * self.trade_amount_percentage)
            if self.dif_dif_sma > self.buy_threshold:
                if self.is_buy:
                    self.order_buy = self.buy(size=self.trade_amount)
            if self.dif_dif_sma < self.sell_threshold:
                if self.is_sell:
